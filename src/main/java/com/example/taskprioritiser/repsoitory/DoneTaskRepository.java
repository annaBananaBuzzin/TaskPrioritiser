...
public interface DoneTaskRepository extends JpaRepository<DoneTaskEntity, Long> {
    ...
    @Query(value = "SELECT t.*, td.done_at FROM task_done td INNER JOIN task t ON task_id = t.task_id WHERE t.task_id = :taskId", nativeQuery = true)
    DoneTaskEntity fetchDoneTaskByTaskId(@Param("id") Long taskID);

    @Query(value = "SELECT t.*, td.done_at FROM task_done FROM task_done td INNER JOIN task t ON task_id = t.task_id WHERE t.description = :description", nativeQuery = true)
    DoneTaskEntity fetchDoneTaskByTaskDescription(@Param("description") String description);


    @Query(value = "SELECT t.*, td.done_at FROM task_done td INNER JOIN task t ON td.task_id = t.task_id", nativeQuery = true)
    List<DoneTaskEntity> fetchAllDoneTasks();
